    ...
    def complete(self, prompt: str, caller: str = "device_complete", coalition_id: int = 0) -> str:
        ...
        if cache_path and cache_path.exists():
            with open(cache_path, encoding="utf-8") as f:
                ...
                # Cache hit: DO NOT add inference tokens! 0 tokens added on cache hit.
        ...
        if self.config.get("use_mock", True):
            before_mem, _ = tracemalloc.get_traced_memory() if tracemalloc.is_tracing() else (0, 0)
            ...
            self.usage.api_calls = self.usage.device_api_calls
            self.usage.successful_calls += 1
            self.usage.memory_mb = psutil.Process(os.getpid()).memory_info().rss / (1024 * 1024)

            after_mem, _ = tracemalloc.get_traced_memory() if tracemalloc.is_tracing() else (0, 0)
            delta = max(0.0, (after_mem - before_mem) / (1024 * 1024))
            self.usage.python_heap_delta_mb = max(self.usage.python_heap_delta_mb, delta)
...
def aggregate_device_usage(device_llms: dict[str, DeviceLLMClient]) -> DeviceLLMUsage:
    ...
    total.python_heap_delta_mb = max((c.usage.python_heap_delta_mb for c in device_llms.values()), default=0.0)
    total.python_heap_delta_by_device = {k: c.usage.python_heap_delta_mb for k, c in device_llms.items()}
