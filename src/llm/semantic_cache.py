...
class SemanticPlanCache:
    ...
    def lookup(
        ...
        self.cache_misses += 1
        if not any_within_age:
            self.miss_reason_log.append({"step": current_step, "reason": "no_candidate_within_age", "entries_count": len(self.entries), "max_cache_age": self.max_cache_age})
        else:
            self.miss_reason_log.append({"step": current_step, "reason": "similarity_below_threshold", "best_similarity": round(best_sim, 6), "threshold": self.similarity_threshold})
        return None
